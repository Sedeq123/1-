...
# ==========================================
# 2. تبويب توليد التصاميم المرن (تحديث بناءً على ملاحظتك)
# ==========================================
with tabs[1]:
    ...
    user_specs = st.text_area("✍️ اشرح التصميم المطلوب بكلماتك (اختياري):", 
                              placeholder="مثال: أريد سجادة تراثية بنقوش إبل وصقور مع دمج خيوط الحرير الذهبي...")
    ...
    if st.button("✨ ابدأ توليد التصميم الإبداعي"):
        # التحقق من أن المستخدم أدخل خياراً واحداً على الأقل (نص أو صورة)
        if user_specs or uploaded_file:
            st.info("🔄 جاري معالجة طلبك بمرونة تامة عبر محرك الذكاء الاصطناعي...")
            st.success("✨ تم توليد التصميم الجديد بناءً على مدخلاتك الخاصة بنجاح!")
            
            # عرض تفاصيل ما تم استخدامه
            if user_specs and not uploaded_file:
                st.caption(f"📝 تم التوليد بناءً على وصفك النصي: {user_specs}")
            elif uploaded_file and not user_specs:
                st.caption("🖼️ تم التوليد كنسخة مطورة ومستوحاة من الصورة المرفوعة.")
            else:
                st.caption(f"بناءً على دمج الوصف النصي والصورة الفنية المرفقة.")
                
            # عرض نموذج فني تخيلي للنتيجة
            st.image("https://images.unsplash.com/photo-1600121848594-d8644e57abab?q=80&w=600", 
                     caption="نموذج السجادة المولد بالذكاء الاصطناعي لمعمل نسيج الذكي", width=500)
        else:
