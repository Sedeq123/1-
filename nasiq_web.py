...
# تخصيص واجهة الويب لتدعم الكتابة من اليمين إلى اليسار وتعديل مظهر الأزرار
st.markdown("""
    <style>
...
# تفعيل حالة التوليد لضمان استقرار عرض الصور
if 'generated' not in st.session_state:
    st.session_state.generated = False
if 'gen_type' not in st.session_state:
    st.session_state.gen_type = ""
if 'carpet_img_url' not in st.session_state:
    st.session_state.carpet_img_url = "https://images.unsplash.com/photo-1600121848594-d8644e57abab?q=80&w=600"

data = st.session_state.app_data

st.title("🧶 لوحة تحكم إمبراطورية 'نسيج الذكي'")
st.write("مرحباً بك يا صديقي! هذه هي واجهة الويب التفاعلية لإدارة معملك وتصاميمك الملحمية التنافسية.")
st.markdown("---")

# تقسيم الشاشة إلى تبويبات رئيسية لسهولة التنقل
tabs = st.tabs(["🎙️ الأوامر الصوتية", "📸 استلهام وتوليد التصاميم", "👥 الموارد البشرية (HR)", "👑 إدارة زبائن النخبة", "💡 صندوق الأفكار"])
...
# ==========================================
# 2. تبويب توليد التصاميم المرن المحسن والمبهر
# ==========================================
with tabs[1]:
    st.header("📸 ميزة التوليد والاستلهام الذكي والمرن")
    st.write("أنت مخيّر بالكامل هنا؛ يمكنك الشرح كتابةً فقط، أو رفع صورة فقط، أو استخدام الاثنين معاً:")
    
    user_specs = st.text_area("✍️ اشرح التصميم المطلوب بكلماتك (اختياري):", 
                              placeholder="مثال: أريد تصميم سجاد لون ذهبي مع اسود...")
    
    uploaded_file = st.file_uploader("📸 أو اختر صورة من الألبوم / التتقطها بالكاميرا للاستلهام (اختياري):", type=["jpg", "png", "jpeg"])
    
    # قائمة بروابط صور سجاد شرقي وفاخر جداً ومزخرف بدقة عالية لتليق بالمعمل
    carpet_pool = [
        "https://images.unsplash.com/photo-1600121848594-d8644e57abab?q=80&w=600", # سجادة زخارف ملكية فاخرة
        "https://images.unsplash.com/photo-1579783900882-c0d3dad7b119?q=80&w=600", # سجاد فنون تجريدية دافئة
        "https://images.unsplash.com/photo-1562575214-da9fcf59b907?q=80&w=600", # تفاصيل خيوط النسيج الفاخرة
        "https://images.unsplash.com/photo-1594498653385-d5172b532c00?q=80&w=600"  # نقوش هندسية كلاسيكية ملحمية
    ]
    
    if st.button("✨ ابدأ توليد التصميم الإبداعي"):
        if user_specs or uploaded_file:
            st.session_state.generated = True
            # اختيار صورة سجاد حقيقية ومبهرة عشوائياً لكسر جمود الرابط الثابت
            st.session_state.carpet_img_url = random.choice(carpet_pool) + f"?cb={random.randint(1,1000)}"
            
            if user_specs and not uploaded_file:
                st.session_state.gen_type = f"📝 تم التوليد المحاكي بناءً على وصفك الفعلي: {user_specs}"
            elif uploaded_file and not user_specs:
                st.session_state.gen_type = "🖼️ تم استلهام الأنماط الفنية من الصورة المرفوعة وتحديث خط الإنتاج."
            else:
                st.session_state.gen_type = "🤝 تم دمج خطوط الوصف النصي مع الصورة الملهمة لإنتاج نمط السجادة النادر."
        else:
            st.session_state.generated = False
            st.warning("⚠️ الرجاء كتابة وصف نصي أو رفع صورة أولاً لكي يستطيع الذكاء الاصطناعي البدء بالتوليد.")

    # عرض النتائج الفاخرة
    if st.session_state.generated:
        st.markdown("---")
        st.info("🔄 جاري معالجة الأنماط ودمج الخيوط الفاخرة عبر محرك نسيج الذكي...")
        st.success("✨ تم صياغة وتوليد نمط السجادة الجديد ليكون جاهزاً للنحت والتشطيب والمنافسة!")
        st.caption(st.session_state.gen_type)
        
        # عرض صورة السجادة الفاخرة التي تم اختيارها وتحديثها
        st.image(st.session_state.carpet_img_url, 
                 caption="النمط الفني الفاخر المولد حالياً لمعمل نسيج الذكي", width=500)
...
with tabs[2]:
    ...
    with col1:
        ...
        if st.button("تأكيد وإسناد المهمة"):
            ...
            task = {"design": selected_design, "area": area, "payout": total_pay, "status": "قيد النحت والتشطيب"}
            data["hr"][worker_id]["tasks"].append(task)
            st.success(f"تم إسناد المهمة بنجاح للفني! الأجر المحسوب والمضاف لحسابه: {total_pay} ريال.")
    ...
    with col2:
        ...
        for k, v in data["hr"].items():
            ...
            if v["tasks"]:
                for t in v["tasks"]:
                    st.caption(f"📍 تصميم: {t['design']} | المساحة: {t['area']}م٢ | الأجر المستحق: {t['payout']} ريال ({t['status']})")
            else:
...
with tabs[3]:
    ...
    with col3:
        ...
        comment = st.text_area("رأي وانطباع الزبون الفعلي:")
        
        if st.button("حفظ وتوثيق رأي الزبون"):
            cust_id = selected_cust.split(" - ")[0]
            if "orders" not in data["crm"][cust_id]:
                data["crm"][cust_id]["orders"] = []
            data["crm"][cust_id]["orders"].append(cust_design)
            data["crm"][cust_id]["feedback"].append({"product": _cust_design, "rating": rating, "comment": comment})
...
with tabs[4]:
    ...
    if st.button("حفظ الفكرة في صندوق الأفكار"):
        if new_suggestion:
            ...
            st.success("تم حفظ فكرتك الإبداعية بنجاح في صندوق مقترحاتك الشخصية!")
        else:
