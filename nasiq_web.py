...
st.write("مرحباً بك يا صديقي! هذه هي واجهة الويب التفاعلية لإدارة معملك وتصاميمك الملحمية.")
st.markdown("---")
...
# ==========================================
# 2. تبويب توليد التصاميم المرن المحسن
# ==========================================
with tabs[1]:
    ...
    uploaded_file = st.file_uploader("📸 أو اختر صورة من الألبوم / التقطها بالكاميرا للاستلهام (اختياري):", type=["jpg", "png", "jpeg"])
    
    if st.button("✨ ابدأ توليد التصميم الإبداعي"):
        if user_specs or uploaded_file:
            st.session_state.generated = True
            if user_specs and not uploaded_file:
                st.session_state.gen_type = f"📝 تم التوليد بناءً على وصفك النصي الفعلي: {user_specs}"
            elif uploaded_file and not user_specs:
                st.session_state.gen_type = "🖼️ تم التوليد كنسخة مطورة ومستوحاة من الصورة المرفوعة بنجاح."
            else:
                st.session_state.gen_type = "🤝 تم التوليد بناءً على دمج الوصف النصي والصورة الفنية المرفقة معاً."
        else:
            st.session_state.generated = False
            st.warning("⚠️ الرجاء كتابة وصف نصي أو رفع صورة أولاً لكي يستطيع الذكاء الاصطناعي البدء بالتوليد.")

    # عرض النتائج بشكل منفصل لمنع تعليق المتصفح
    if st.session_state.generated:
        st.markdown("---")
        st.info("🔄 جاري معالجة طلبك بمرونة تامة عبر محرك الذكاء الاصطناعي...")
        st.success("✨ تم توليد التصميم الجديد بناءً على مدخلاتك الحالية بنجاح!")
        st.caption(st.session_state.gen_type)
        
        # رابط صورة متجدد لكسر الذاكرة المؤقتة للمتصفح وضمان التحديث
        st.image("https://images.unsplash.com/photo-1583847268964-b28dc8f51f92?q=80&w=600", 
                 caption="شكل السجادة الفاخرة المولد حالياً لمعمل نسيج الذكي", width=500)
